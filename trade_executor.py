...
# FIX A: order_id extracted from result["order"]["order_id"] after place_limit_order
# FIX B: actual_entry from deribit.get_fill_price(result, fallback)
# FIX C: is_order_filled() uses deribit.is_order_filled() helper (order_state == "filled")
# FIX D: _fill_price() reads order dict correctly from get_order response
# FIX E: clear_stuck_trades() detects missing/empty SL order_ids from old bugs
...
def save_json(path, data):
    # Write to root AND data/ so both GitHub Actions cache and repo folder stay in sync
    for dest in [path, str(Path("data") / Path(path).name)]:
        try:
...
def execute_trade(deribit: DeribitClient, symbol, signal, entry, atr,
    ...
    if not deribit.is_supported(symbol):
        log.info(f"  {symbol}: not on Deribit testnet — skip"); return False
    ...
    try:
        # ── Entry ─────────────────────────────────────────────────────
        entry_result = deribit.place_market_order(symbol, side, amount)
        if not entry_result:
            log.error(f"  Entry returned empty — skip"); return False

        # FIX A+B: extract order_id from result["order"], fill price from result["trades"]
        entry_order = entry_result.get("order", entry_result)
        order_ids["entry"] = str(entry_order.get("order_id", ""))
        actual_entry = deribit.get_fill_price(entry_result, entry)   # FIX B
        if actual_entry == 0: actual_entry = entry
        log.info(f"  ✅ Entry @ ~{actual_entry:.{dec}f}")
        time.sleep(1.5)

        # Recalculate SL/TP from actual fill price to avoid overlap errors
        if signal == "BUY":
        ...
        # ── Stop Loss ─────────────────────────────────────────────────
        try:
            sl_result = deribit.place_limit_order(symbol, sl_side, amount, stop, stop_price=stop)
            sl_order  = sl_result.get("order", sl_result)          # FIX A
        ...
        try:
            if amount_tp1 > 0:
                ...
                tp1_order  = tp1_result.get("order", tp1_result)   # FIX A
        ...
        try:
            if amount_tp2 > 0:
                ...
                tp2_order  = tp2_result.get("order", tp2_result)   # FIX A
    ...
    _send_open_alert(symbol, signal, confidence, score, actual_entry,
                     stop, tp1, tp2, amount, amount_tp1, amount_tp2, risk_usd, balance, reasons, risk_mult)
...
def _fill_price(order: dict, fallback: float) -> float:
    # FIX D: get_order returns a plain order dict (not the full result with trades[])
    # Use average_price, then price field
...
def check_open_trades(deribit: DeribitClient):
    ...
    for symbol, trade in list(trades.items()):
        ...
        def get_o(key):
            if key not in oids or not oids[key] or oids[key] in ("", "None"): return {}
        ...
        try:
            ...
            if not trade["tp1_hit"] and "tp1" in oids:
                o = get_o("tp1")
                if deribit.is_order_filled(o):    # FIX C
                    trade["tp1_hit"] = True
                    ...
                    if "stop_loss" in oids and oids["stop_loss"]:
                        try:
            ...
            if trade.get("tp1_hit") and not trade.get("tp2_hit") and oids.get("stop_loss"):
                ...
                if live > 0:
                    ...
                    is_bull   = trade["signal"] == "BUY"  and live >= halfway
                    is_bear   = trade["signal"] == "SELL" and live <= halfway
                    at_be     = abs(trade.get("stop", 0) - entry) < 0.01 * entry
                    if (is_bull or is_bear) and at_be:
                        try:
                            deribit.cancel_order(oids["stop_loss"])
                            sl_side = "SELL" if trade["signal"] == "BUY" else "BUY"
                            qty_rem = trade.get("qty_tp2", trade.get("qty_tp1", 1))
                            sl_res  = deribit.place_limit_order(symbol, sl_side, qty_rem, trade["tp1"], stop_price=trade["tp1"])
                            sl_o    = sl_res.get("order", sl_res)
                            new_id  = str(sl_o.get("order_id", ""))
                            if new_id:
                                trade["order_ids"]["stop_loss"] = new_id
                                trade["stop"] = trade["tp1"]
                            log.info(f"  🚀 {symbol} trailing stop → TP1 ({trade['tp1']})")
                            _send(f"🚀 *{symbol}* Trailing stop → profit `{trade['tp1']:.{dec}f}`")
                        except Exception as e:
                            log.warning(f"  Trailing SL failed: {e}")
...
def clear_stuck_trades(deribit: DeribitClient):
    """FIX E: Remove broken trades with missing SL/TP order IDs from old bugs."""
    trades = load_trades()
    ...
    for symbol in list(trades.keys()):
        ...
        # Detect broken: SL or TP1 missing/empty (from old _post → _get bug)
        sl  = oids.get("stop_loss", "")
        tp1 = oids.get("tp1", "")
        if not sl or sl in ("", "None") or not tp1 or tp1 in ("", "None"):
            log.warning(f"  ⚠️ {symbol}: missing SL/TP IDs — clearing broken trade")
            _record_close(trade, float(trade.get("entry", 0)), 0.0,
                          "Cleared: SL/TP not placed (old POST bug now fixed)")
            trades.pop(symbol); cleared += 1; continue

        # Check if live orders still exist on exchange
        live = False
...
def _send_open_alert(symbol, signal, confidence, score, entry, stop, tp1, tp2,
    ...
    _send(
        ...
        f"📦 Total: *{amount}* · Risk: `${risk_usd:.2f}`\n"
...
def run_diagnostic():
    ...
    try:
        ...
        lines += [
            ...
            f"📋 Tradeable: {len(TRADEABLE_SYMBOLS)} — {TRADEABLE_SYMBOLS}",
        ]
    except Exception as e:
    ...
    lines += [
        ...
        f"⚙️ Conf≥{mode['min_confidence']}% Score≥{mode['min_score']}",
        f"⚡ Risk: *{int(eff*100)}%* of {int(RISK_PER_TRADE*100)}%",
    ]
    try:
    ...
    lines += [
        f"📈 Win rate: *{wr}%* ({len(wins)}W/{len(real)-len(wins)}L/{len(real)})",
        f"💵 PnL: *{tpnl:+.4f}*",
    ]
    _send("\n".join(lines))
