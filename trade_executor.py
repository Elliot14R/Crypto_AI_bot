# trade_executor.py — PRO MASTER VERSION
...
def execute_trade(ex, symbol, signal, entry, atr, confidence, score, reasons):
    ...
    try:
        ...
        # Stop loss
        try:
            sl_o = ex.create_order(symbol, "stop_loss_limit", sl_side, qty, stop, params={"stopPrice": stop, "timeInForce": "GTC"})
            order_ids["stop_loss"] = sl_o["id"]
        except Exception as e: log.warning(f"  SL fail: {e}")

        # TP1 & TP2
        try:
            tp1_o = ex.create_order(symbol, "take_profit_limit", tp_side, qty_tp1, tp1, params={"stopPrice": tp1, "timeInForce": "GTC"})
            order_ids["tp1"] = tp1_o["id"]
        except Exception as e: pass
        try:
            tp2_o = ex.create_order(symbol, "take_profit_limit", tp_side, qty_tp2, tp2, params={"stopPrice": tp2, "timeInForce": "GTC"})
        ...
        for sym in active_symbols:
            if sym not in trades:
                ...
                trades[sym] = {
                    ...
                    "stop": 0, "tp1": 0, "tp2": 0, "qty": total_qty,
                    "qty_tp1": total_qty / 2, "qty_tp2": total_qty / 2,
                    "risk_usd": 0, "balance_at_open": 0, "order_ids": order_ids,
                    "tp1_hit": False, "tp2_hit": False, "closed": False,
            ...
            if trade["tp1_hit"] and not trade["tp2_hit"] and "tp2" in oids:
                try:
                    ...
                    if o["status"] == "closed":
                        ...
                        _send_close_alert(symbol, "✅ FULL WIN", pnl, entry, avg_price, trade["opened_at"])
...
def _pnl(trade, close_price, close_type):
    entry = trade["entry"]; qty = trade["qty_tp1"] if close_type == "tp1" else trade["qty_tp2"] if close_type == "tp2" else trade["qty"]
    return round((close_price - entry) * qty, 4) if trade["signal"] == "BUY" else round((entry - close_price) * qty, 4)
...
def generate_signal(symbol, pipeline, thresholds):
    try:
        ...
        if float(row_entry.get("adx", 0)) < thresholds["min_adx"]: return None

        score, reasons = _quality_score(row_entry, row_confirm, signal, confidence)
        entry = float(row_entry["close"]); atr = float(row_entry["atr"])
        
        if score < thresholds["min_score"]: return None
        return {"symbol": symbol, "signal": signal, "confidence": confidence, "score": score, "entry": entry, "atr": atr, "reasons": reasons}
...
def _quality_score(row_entry, row_confirm, signal, confidence):
    ...
    elif adx > 18: score += 1; reasons.append(f"Moderate trend ADX {adx:.0f}")
...
def run_execution_scan():
    ...
    # 🧹 FREE THE MONEY FIRST
    clean_ghost_orders(exchange)

    # Reconstruct data if lost
    ...
    for symbol in SYMBOLS:
        ...
        if vol.get("warn"): sig["reasons"].append(f"⚠️ {vol['message']}")
