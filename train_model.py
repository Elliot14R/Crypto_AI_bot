# train_model.py — The True Hybrid (High Frequency + High Precision)
...
# ── 1. The Picklable Importance Selector (Crucial for live_scanner.py)
class ImportanceSelector(BaseEstimator, TransformerMixin):
    def __init__(self, feature_names):
        self.feature_names = feature_names
    def fit(self, X, y=None): return self
    def transform(self, X):
        if isinstance(X, pd.DataFrame): return X[self.feature_names]
        return X
...
LIMIT        = 1500
TARGET_BARS  = 6
TARGET_PCT   = 0.003  # Keeps trade frequency HIGH
TEST_SPLIT   = 0.20
...
def train(dataset):
    ...
    # ── 4. Training (Without distorting already balanced data) ──────────
